...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ИсточникВыбора) = Тип("ФормаКлиентскогоПриложения")
		И ИсточникВыбора.ИмяФормы = "Документ.РасходДенежныхСредств.Форма.ФормаСписокПокупок" Тогда
		
		ЗагрузитьСписокПокупокИзВременногоХранилища(ВыбранноеЗначение);
		Модифицированность = Истина;
		УстановитьНадписьСпискаПродуктов();
...
&НаКлиенте
Процедура ОткрытьСписокПокупок(Команда)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("СписокПокупок", ПоместитьСписокПокупокВоВременноеХранилище());
	
	ОткрытьФорму("Документ.РасходДенежныхСредств.Форма.ФормаСписокПокупок", ПараметрыФормы, ЭтаФорма);
...
&НаСервере
Процедура ЗагрузитьСписокПокупокИзВременногоХранилища(ВыбранноеЗначение)
	
	Объект.СписокПокупок.Загрузить(ПолучитьИзВременногоХранилища(ВыбранноеЗначение));
	
	Объект.Сумма = Объект.СписокПокупок.Итог("Сумма");
...
Функция ПоместитьСписокПокупокВоВременноеХранилище()
	...
	Возврат ПоместитьВоВременноеХранилище(Объект.СписокПокупок.Выгрузить());
...
Процедура ПодтвердитьФактическийРасходЗавершение()
	...
	Объект.Дата = ОбщегоНазначенийВызовСервера.ПолучитьИспользуемуюДату();
	НачатьЗаписьДокумента();
...
Функция ПроверитьПревышениеЛимитаЗаПериод()
	...
	ПланируемыеРасходы = УчетДенежныхСредствСервер.ПолучитьПланируемыеРасходыЗаПериод(ДатаНачалаПериода, ДатаОкончанияПериода);
...
&НаСервере
Процедура УстановитьНадписьСпискаПродуктов()
	
	Элементы.ОткрытьСписокПокупок.Заголовок = "Список покупок (" + Объект.СписокПокупок.Количество() + ")";
